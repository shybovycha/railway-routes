...
function Map(app, bridge) {
    ...
    this.bridge = bridge;
    this.ns = '';
    // Global website URL should go here.
    // E. g. the hostname and the port (if it is not 80) where the server is running.
    this.globPath = '/';
    this.pathTo = {};
    this.dump = [];
    this.middlewareStack = [];
    this.noJugglingDB = ((this.app.set('no-jugglingdb') == false) || true);
}
...
Map.prototype.addPath = function (templatePath, action, helperName) {
    ...
    this.pathTo[helperName] = function (objParam) {
        // TODO: think about removing or rewriting it
        ...
        for (var i = 0; i < paramsLength; i += 1) {
            value = null;
            arg = arguments[i];
            if (arg && 'function' === typeof arg.to_param) {
                // This line pastes arg code (if arg is a function) to the URL.
                // This is a very serious vulnerability.
                // Skipping this parameter handling is a good idea.
                //   value = arg.to_param();
            } else if (paramNames[i] && objParam && objParam[paramNames[i]]) {
                value = objParam[paramNames[i]];
            } else if (arg && typeof arg === 'object') {
                if (this.noJugglingDB == false && arg.id && arg.constructor.name !== 'ObjectID') {
                    value = arg.id;
                } else if (this.noJugglingDB == true) {
                    if (arg.id) {
                        value = arg.id;
                    } else if (arg._id) {
                        value = arg._id;
...
Map.prototype.resources = function (name, params, actions) {
    ...
    function getActiveRoutes(params) {
        ...
        // 2. except
            ...
            availableRoutes.forEach(function(action) {
